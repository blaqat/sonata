...
def _deliver_image(image_bytes: bytes, mime_type: str = None) -> str:
    ...
    usable link, but the bytes are what actually reach the channel. When the
    upload fails we can still attach, so the generation only fails outright if
    there is no channel to attach to.
    """
...
async def reply_with_generated_images(ctx, channel_id, text):
    """Send ``text``, attaching any images generated during this turn.
    ...
    keeping the hosted URL as the fallback path.
    """
    images = image_delivery.take(channel_id)
    if not images:
...
async def ai_question(ctx, *message, ai, short, error_prompt=None):
    ...
    try:
        ...
        async with ctx.typing():
            ...
            r = await reply_with_generated_images(ctx, channel.id, r)
