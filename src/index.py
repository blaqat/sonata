...
from modules.emoji_utils import download_emoji, read_images, trans_emo
...
def _deliver_image(image_bytes: bytes, mime_type: str = None) -> str:
    """Queue generated bytes for Discord and return the text the AI should use.

    Catbox stays in the path so chat history and non-Discord callers keep a
    usable link, but inside a delivery turn the bytes are what actually reach
    the channel. Without an open turn (terminal and voice replies) the hosted
    link is the only route, so an upload failure still raises there.
    """
    filename = f"generated.{image_delivery.extension_for(mime_type)}"
    try:
        url = upload_to_catbox(image_bytes)
    except CatboxUploadError:
        if not image_delivery.stash(image_bytes, filename):
            raise
        return image_delivery.ATTACHED_NOTICE

    image_delivery.stash(image_bytes, filename, url=url)
    return url
...
def DallE(client, prompt, model, config):
    ...
    return _deliver_image(image_bytes, "image/png")
...
def NanoBanana(client, prompt, model, config):
    ...
    return _deliver_image(inline_data.data, getattr(inline_data, "mime_type", None))
...
async def ctx_reply(ctx, r, reply=True, files=None):
    """
    Replies to the context, handling both message and interaction contexts.
    """
    content = str(r or "")[:2000] or None
    ...
    try:
        _ = ctx.author
        if reply:
            await ctx.reply(content, mention_author=False, **extra)
        else:
            await ctx.send(content, **extra)
    except AttributeError as _:
        cprint(content or "", "red")
        await ctx.send(content, **extra)
...
async def reply_with_generated_images(ctx, images, text):
    """Send ``text``, attaching ``images`` generated during this turn.

    Returns the text actually sent so chat history matches the message. If
    Discord rejects the upload the original link-bearing text goes out instead,
    keeping the hosted URL as the fallback path. When an image has no hosted
    link either, say so rather than leaving the reply claiming an attachment
    that never arrived.
    """
    if not images:
        await ctx_reply(ctx, text)
        return text

    attached_text = image_delivery.strip_urls(text, images)
    try:
        await ctx_reply(
            ctx,
            attached_text,
        ...
        cprint(f"Image attachment failed, falling back to link: {e}", "yellow")

    if any(image.url is None for image in images):
        text = f"{text}\n\n{image_delivery.DELIVERY_FAILED_NOTICE}".strip()
    await ctx_reply(ctx, text)
    return text
...
async def ai_question(ctx, *message, ai, short, error_prompt=None):
    ...
    try:
        ...
        async with ctx.typing():
            # Opened here, not in chat.request: this is the only path that can
            # turn generated bytes into attachments, and each concurrent reply
            # needs its own turn so images land on the message that made them.
            with image_delivery.delivery_turn() as turn:
            ...
            r = await reply_with_generated_images(ctx, images, r)
